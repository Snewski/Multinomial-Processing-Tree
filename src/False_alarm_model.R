...
#stim <- as.integer(str_sub(dat$Video,-1))
...
samples <- jags(data, inits=NULL, params,
                model.file ="Models/MPT_false_alarm2.txt",
                n.chains=3, n.iter=5000, n.burnin=1000, n.thin=1)

png("results/false_alarm_results.png", width = 800, height = 800)   # open PNG device
par(mfrow=c(2,2), oma=c(0,0,2,0))
plot(samples$BUGSoutput$sims.list$kappa_A,
     samples$BUGSoutput$sims.list$gamma_A,
     xlim=c(0,1),ylim=c(0,1),
     col=rgb(.1,.8,.1,.1),pch = 16,
     xlab = "Knowledge",ylab = "Guessing",main = "A) Highest Free Man")
...
plot(samples$BUGSoutput$sims.list$kappa_B,
     ...
     xlim=c(0,1),ylim=c(0,1),
     col=rgb(.1,.8,.1,.1),pch = 16,
     xlab = "Knowledge",ylab = "Guessing",main = "B) Link Out")
...
plot(samples$BUGSoutput$sims.list$kappa_C,
     ...
     xlim=c(0,1),ylim=c(0,1),
     col=rgb(.1,.8,.1,.1),pch = 16,
     xlab = "Knowledge",ylab = "Guessing",main = "C) Switch Play to Open Lines")
...
plot(samples$BUGSoutput$sims.list$kappa_D,
     ...
     xlim=c(0,1),ylim=c(0,1),
     col=rgb(.1,.8,.1,.1),pch = 16,
     xlab = "Knowledge",ylab = "Guessing",main = "D) Pass it Over the Opponent")
points(mean(samples$BUGSoutput$sims.list$kappa_D),
       mean(samples$BUGSoutput$sims.list$gamma_D),
       col="red",pch=13)

mtext("False-alarm model", outer=TRUE, cex=1.5)
